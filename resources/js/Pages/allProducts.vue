  ...
  <div class="container-fluid px-0 bg-gray-700">
    ...
    <div v-if="allProducts.data.length > 0">
        <div class="grid md:grid-cols-6 grid-cols-2 m-3">
            <div dir="ltr" v-for="(prod, index) in allProducts.data" :key="index" class="hvr-bob card bg-gray-700 border-2 border-gray-400 rounded-lg mx-2 mb-3">
    ...
    <!--  Pagination  -->
    <div v-if="allProducts" class="flex justify-center my-3">
        <inertia-link v-if="allProducts.current_page > 2"
            :href="allProducts.first_page_url">
            <span class="rounded-2 px-2 py-1 text-white ml-1 hover:bg-red-900 bg-red-600">
                <fa icon="angle-double-right" />
            </span>
        </inertia-link>
        <pagination :links="allProducts.links" />
        <inertia-link v-if="allProducts.current_page < allProducts.last_page - 2"
            :href="allProducts.last_page_url">
            <span class="rounded-2 px-2 py-1 text-white ml-1 hover:bg-red-900 bg-red-600">
